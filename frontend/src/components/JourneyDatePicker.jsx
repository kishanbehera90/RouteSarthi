    ...
    <div ref={wrapRef} className="relative">
      <button
        ...
        className={cn(
          ...
          open ? 'border-brand-600 ring-2 ring-brand-600/15' : 'border-brand-900/10 hover:border-brand-900/20'
        )}
      >
        ...
        <span className="min-w-0 flex-1">
          ...
          <span className={cn('block text-sm font-semibold', selected ? 'text-brand-900' : 'text-gray-400')}>
      ...
      {open && (
        <div className="absolute left-0 right-0 top-full z-30 mt-2 rounded-2xl border border-brand-900/10 bg-white p-4 shadow-xl shadow-brand-900/10 sm:right-auto sm:w-80">
          <div className="flex items-center justify-between">
            <button
              type="button"
              onClick={() => stepMonth(-1)}
              disabled={!canPrev}
              className="flex h-8 w-8 items-center justify-center rounded-lg text-brand-700 transition hover:bg-sand-100 disabled:opacity-30"
              aria-label="Previous month"
            >
              <ChevronLeft className="h-4 w-4" />
            </button>
            <p className="font-display text-sm font-bold text-brand-900">{monthFmt.format(view)}</p>
            <button
              type="button"
              onClick={() => stepMonth(1)}
              disabled={!canNext}
          ...
          <div className="mt-3 grid grid-cols-7 gap-1 text-center">
            {weekdays.map((w) => (
              <span key={w} className="text-[10px] font-semibold uppercase tracking-wide text-gray-400">
                {w}
              </span>
            ))}
          </div>
              ...
              c === null ? (
                ...
                <button
                  ...
                  className={cn(
                    ...
                    c.iso === value && 'bg-brand-900 text-white hover:bg-brand-900'
                  )}
                >
                  ...
                  {!c.disabled && (
                    <span
                      className={cn(
                        ...
                        c.iso === value ? 'bg-white/80' : dotStyles[c.level]
                      )}
                    />
                  )}
                </button>
          ...
          <div className="mt-2 flex items-start gap-1.5 rounded-xl bg-sand-100 px-3 py-2 text-xs text-brand-800">
