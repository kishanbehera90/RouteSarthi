        ...
        {toasts.map((t) => {
          ...
          return (
            <motion.div
              ...
              className="pointer-events-auto flex items-center gap-2.5 rounded-2xl border border-brand-900/10 bg-white px-4 py-3 shadow-lg shadow-brand-900/10"
            >
              <Icon className={cn('h-5 w-5 shrink-0', accent)} />
              <span className="text-sm font-medium text-brand-900">{t.message}</span>
              <button
                type="button"
                onClick={() => dismiss(t.id)}
                aria-label="Dismiss"
                className="ml-1 text-gray-300 transition hover:text-brand-700"
              >
                <X className="h-4 w-4" />
              </button>
            </motion.div>
