"""Seed fixtures ported from frontend/src/data/routes.js.

Step-0 parity layer: the backend serves the same data the frontend mock does,
so the API contract is proven end-to-end with a real server before any real
data or routing logic exists. Step 1 replaces these fixtures with rows loaded
from Postgres/PostGIS + the cross-origin engine.
"""
...
ROUTES_BY_CORRIDOR = {
    "rourkela-nashik": [
        {
            ...
            "legs": [
                {
                    "id": "ron-d1-l1", "mode": "train", "name": "18030 Rourkela–LTT Express",
                    "from": "Rourkela", "to": "Nashik Road", "depart": "21:40", "arrive": "05:10+2",
                    "durationMins": 32 * 60 - 30, "fareInr": 450, "confirmation": "waitlisted",
                    "waitlistPosition": 38, "clearProbabilityPct": 18,
        ...
        },
        {
            ...
            "legs": [
                {
                    "id": "ron-c1-l1", "mode": "bus", "name": "Rourkela → Ranchi (State Highway Express)",
                    "from": "Rourkela Bus Stand", "to": "Ranchi Bus Stand", "depart": "15:30", "arrive": "17:35",
                    "durationMins": 125, "fareInr": 220, "confirmation": "confirmed",
                    "delayProfile": {"avgMins": 12, "onTimePct": 88},
                },
                {"id": "ron-c1-l2", "mode": "connection", "connectionSafetyPct": 94, "bufferMins": 95,
                 "note": "Bus historically arrives with ~95 min to spare before departure — high safety margin."},
                {
                    "id": "ron-c1-l3", "mode": "train", "name": "12810 Ranchi–LTT SF Express",
                    "from": "Ranchi", "to": "Nashik Road", "depart": "19:10", "arrive": "18:00+1",
                    "durationMins": 23 * 60 + 10, "fareInr": 560, "confirmation": "confirmed",
        ...
        },
        {
            ...
            "legs": [
                {
                    "id": "ron-c2-l1", "mode": "bus", "name": "Rourkela → Ranchi (Express)",
                    "from": "Rourkela Bus Stand", "to": "Ranchi Bus Stand", "depart": "13:00", "arrive": "15:05",
                    "durationMins": 125, "fareInr": 220, "confirmation": "confirmed",
                    "delayProfile": {"avgMins": 12, "onTimePct": 88},
                },
                {"id": "ron-c2-l2", "mode": "connection", "connectionSafetyPct": 88, "bufferMins": 65,
                 "note": "Comfortable buffer; this train’s delay profile is slightly higher-variance."},
                {
                    "id": "ron-c2-l3", "mode": "train", "name": "18182 Ranchi–Pune Express",
                    "from": "Ranchi", "to": "Nashik Road", "depart": "16:10", "arrive": "21:10+1",
                    "durationMins": 29 * 60, "fareInr": 470, "confirmation": "confirmed",
    ...
    "bhuj-shimla": [
        {
            "id": "bs-direct-1", "type": "direct", "totalTimeMins": 40 * 60, "totalFareInr": 520,
            "reliability": 18, "confirmation": "waitlisted", "waitlistPosition": 64,
            "confirmationPct": 8, "clearProbabilityPct": 9,
            "why": 'No real direct rail route exists; the only "direct" option is a multi-change unreserved itinerary nobody should rely on.',
            "legs": [
                {
                    "id": "bs-d1-l1", "mode": "train", "name": "19568 Bhuj–Bandra Express (unreserved leg)",
                    "from": "Bhuj", "to": "Ahmedabad", "depart": "20:15", "arrive": "05:40+1",
                    "durationMins": 565, "fareInr": 180, "confirmation": "waitlisted",
                    "waitlistPosition": 64, "clearProbabilityPct": 9,
        ...
        {
            "id": "bs-cross-1", "type": "cross-origin", "totalTimeMins": 33 * 60, "totalFareInr": 1450,
            "reliability": 89, "confirmation": "confirmed", "confirmationPct": 91,
            ...
            "legs": [
                {
                    "id": "bs-c1-l1", "mode": "train", "name": "19568 Bhuj–Bandra Express",
                    "from": "Bhuj", "to": "Ahmedabad", "depart": "20:15", "arrive": "05:40+1",
                    "durationMins": 565, "fareInr": 340, "confirmation": "confirmed",
                    "delayProfile": {"avgMins": 28, "onTimePct": 79},
                },
                {"id": "bs-c1-l2", "mode": "connection", "connectionSafetyPct": 90, "bufferMins": 140,
                 "note": "Plenty of buffer in Ahmedabad to also grab breakfast."},
                {
                    "id": "bs-c1-l3", "mode": "train", "name": "12903 Ahmedabad–Kalka Express",
                    "from": "Ahmedabad", "to": "Kalka", "depart": "08:00", "arrive": "12:20+1",
                    "durationMins": 1700, "fareInr": 890, "confirmation": "confirmed",
                    "delayProfile": {"avgMins": 35, "onTimePct": 74},
                },
                {"id": "bs-c1-l4", "mode": "connection", "connectionSafetyPct": 86, "bufferMins": 70,
                 "note": "Kalka to Shimla toy train has limited daily departures — this buffer is built in."},
                {
                    "id": "bs-c1-l5", "mode": "train", "name": "Kalka–Shimla Toy Train",
                    "from": "Kalka", "to": "Shimla", "depart": "13:30", "arrive": "18:50",
                    "durationMins": 320, "fareInr": 220, "confirmation": "confirmed",
    ...
    "imphal-bengaluru": [
        {
            "id": "ib-direct-1", "type": "direct", "totalTimeMins": 0, "totalFareInr": 0,
            "reliability": 0, "confirmation": "waitlisted", "waitlistPosition": None,
        ...
        {
            "id": "ib-cross-1", "type": "cross-origin", "totalTimeMins": 38 * 60, "totalFareInr": 2350,
            "reliability": 85, "confirmation": "confirmed", "confirmationPct": 88,
            ...
            "legs": [
                {
                    "id": "ib-c1-l1", "mode": "cab", "name": "Imphal → Guwahati (shared cab)",
                    "from": "Imphal", "to": "Guwahati", "depart": "06:00", "arrive": "14:30",
                    "durationMins": 510, "fareInr": 900, "confirmation": "confirmed",
                    "delayProfile": {"avgMins": 35, "onTimePct": 70},
                },
                {"id": "ib-c1-l2", "mode": "connection", "connectionSafetyPct": 91, "bufferMins": 180,
                 "note": "Hill-route cabs run long delays sometimes — this plan books the evening train, not the same-day one, to stay safe."},
                {
                    "id": "ib-c1-l3", "mode": "train", "name": "12508 Guwahati–Bengaluru SF Express",
                    "from": "Guwahati", "to": "Bengaluru (KSR)", "depart": "17:30", "arrive": "23:30+1",
                    "durationMins": 1800, "fareInr": 1450, "confirmation": "confirmed",
...
def get_corridor(corridor_id):
    return next((c for c in CORRIDORS if c["id"] == corridor_id), None)


def get_routes(corridor_id):
    return ROUTES_BY_CORRIDOR.get(corridor_id, [])


def find_route(route_id):
    for routes in ROUTES_BY_CORRIDOR.values():
        for r in routes:
            if r["id"] == route_id:
                return r
    return None
...
def match_corridor(from_query, to_query):
    f = (from_query or "").lower()
    t = (to_query or "").lower()
    return next(
        (c for c in CORRIDORS if f in c["from"]["name"].lower() and t in c["to"]["name"].lower()),
        None,
    )


def weight_routes(routes, pref):
    """pref: 'cheapest' | 'fastest' | 'confirmed' (default)."""
    rs = list(routes)
    if pref == "cheapest":
        rs.sort(key=lambda r: r["totalFareInr"])
    elif pref == "fastest":
        rs.sort(key=lambda r: r["totalTimeMins"])
    else:
        rs.sort(key=lambda r: r["reliability"], reverse=True)
    return rs
